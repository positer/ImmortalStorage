...
> **0.1.3 古玉手册与最终制品同步（2026-08-28）：** 仙窍红石接口条目已在“源与自动化”分类中置于普通/高级仙窍接口之后，并由单页扩写为中英各六页，完整说明资源配置、容器右键、额外资源、正反相施密特逻辑、Enter 保存、重载持久化及低 64/高 256 自动化示例。两代最终离线构建 JAR 均包含该手册，GitHub Release 附件已按最终制品更新；1.21.1 SHA-256 为 `1ABAD7A09635DBB4F2E2929CB3959D9C032FD96B16E351D53CE3F93834BBA80B`，26.1.2 SHA-256 为 `B6F90525F0B41F7C418791722D42AC683E2D6BC61566E154FE1842B3F0A9DEF6`。

> **0.1.3 双版本正式发布（2026-08-28）：** 新增仙窍红石接口、统一机器三态红石控制与机器分类标签，完成 Blockbench 模型、紧凑配置 UI、阈值持久化、红石状态材质及高级仙墟设置布局修复。完整更新说明见 [CHANGELOG 0.1.3](CHANGELOG.md#013---2026-08-28)，下载见 [GitHub Release 0.1.3](https://github.com/positer/ImmortalStorage/releases/tag/0.1.3)。本仓库与 Release 只包含仙藏 `ImmortalStorage`，不包含工作区内其他模组。

> **2026-08-28 0.1.3 红石接口贴图与高级仙墟控件位置修复：** bbmodel 内视觉含义相反的两张嵌入贴图现由导出器显式交换到正确的运行时 activated/inactivated 名称，激活红石/15 亮度状态使用亮起外观，未激活状态使用熄灭外观。高级稳定化迷你仙墟及其纠缠版的红石工作模式按钮已从右侧面板底部移到顶部，数值与调度组件坐标保持不变。两代完整离线构建通过并已部署。

> **2026-08-28 0.1.3 红石接口阈值持久化：** 界面会等待服务端配置同步后再回填高低阈值和反相状态；数字框按主 Enter 或小键盘 Enter 会立即保存 NBT 并重新计算红石输出，重开界面不再被客户端默认零值覆盖。两代完整 `build --offline` 通过并已部署到对应 `ImmortalStorage-*` PCL 实例。

> **2026-08-28 0.1.3 标准方块物品与背包 UI 补丁：** 仙窍红石接口两态 Blockbench 模型现继承原版 `minecraft:block/block`，物品栏、第一/第三人称手持、地面与展示框均使用标准方块展示变换；世界中的 activated/inactivated 方块模型与材质保持不变。配置区压缩为顶部两行，下方加入标准 3×9 玩家背包与 9 格快捷栏。两代完整离线构建通过并部署。

> **2026-08-28 0.1.3 Blockbench/UI/26.1.2 渲染修复：** 仙窍红石接口现以用户 `仙窍红石接口.bbmodel` 为唯一源确定性导出两张原始 PNG 与 0–16 归一化 UV 模型，修复 26.1.2 方块/物品烘焙越界；紧凑 UI 严格保留两行布局，并复用仙窍接口虚拟配置槽的携带物配置、容器右键识别和空槽外部资源弹窗语义。1.21.1 仙墟 renderer 保持不动；26.1.2 在单一 collector 提交中按 1.21.1 顺序绘制外壳与 0.78 倍内核，形成纯屏幕投影轮廓，正反相交换黑白，不绘制物理梁、面框或网络边。两代完整离线构建通过并已部署到 `ImmortalStorage-*` PCL 实例。

> **2026-08-28 0.1.3 UI/模型热修（已被上方修复取代）：** 该轮曾错误判断 64×64 bbmodel 需要 `texture_size`；实机日志证明 26.1.2 会将面 UV 再缩放并越界。最终方案以 bbmodel 自动导出并把像素 UV 归一化到 Minecraft 模型的 0–16 范围。

> **2026-08-28 0.1.3 PCL 专属实例部署：** 两代仙藏实例外层特征标识已统一重命名为 `ImmortalStorage-1.21.1` 与 `ImmortalStorage-26.1.2`，版本 JSON 内 NeoForge 基础 `id` 保持原值。两个实例均已替换为对应 0.1.3 制品且各只保留一个仙藏 JAR；旧实例元数据、配置和 0.1.2 JAR 备份于本地归档，本轮未启动客户端。

> **2026-08-28 0.1.3 开发中：** 已加入机器分类标签、共享三态红石判定与仙窍红石接口。所有 `immortalstorage:machines` 配置页均接入服务端权威三态循环按钮（信号工作 / 无信号工作 / 忽略红石），与机器自身开关执行 AND；旧存档默认忽略红石。接口采用给定 bbmodel 的 activated/inactivated 原始材质，激活时输出 15 红石/15 亮度；右键打开严格两行原版风格 UI，含仙窍接口同源虚拟资源配置槽、空槽额外资源目录、高低阈值与正反相施密特逻辑。兼容门禁强制只读取 PCL 对应实例的已编译 JAR；两代最终完整 `test build --offline` 均通过。
...
> **2026-08-26 合成点击编码与 JEI 配方填充修复：** 1.21.1 实机日志确认 `ServerboundContainerClickPacket` 因 147 个变更槽超过原版 128 上限而断连；空窍/仙窍终端现完全跳过客户端 `clicked` 预测，仅发送原始点击并由服务端执行与同步。JEI 内置合成台和锻造台的 `+` 不再被可能缺失或滞后的客户端材料快照拦截，统一发送服务端权威配方放置请求。两代完整 `build` 通过并替换 `dist/0.1.2`、两个 PCL 实例及 26.1.2 全局 `mods`，未启动客户端。1.21.1 SHA-256 `0427153268625CA908BA8FA94C4666E5E3C1A25643963CD19D1F14D9711B72B4`；26.1.2 SHA-256 `18D2375BE69F79F802F335FDCC3607D9CF3393429A0775D31D80D05A9DDE0884`。详见 `archive/2026-08-26-container-click-jei-transfer-fix.md`。

> **2026-08-26 虚拟鞘翅实际展开与仙墟投影轮廓修复：** 1.21.1 新增 `LocalPlayer.aiStep` 胸甲前置检查桥，使无实体胸甲的仙师度虚拟鞘翅能进入原版 `tryToStartFallFlying` 与发包链；26.1.2 保持其原生直接调用路径，不混入旧版桥。五类迷你仙墟改为有世界深度遮挡的纯色位置几何：外层投影轮廓与内层无纹理、无法线、无明暗和无内部棱线的纯黑/纯白核心。两代完整 `build` 通过并替换 `dist/0.1.2`、两个 PCL 版本实例及 26.1.2 全局 `mods`，未启动客户端。1.21.1 SHA-256 `58F3AA4E952DED7E3EFAEDE050196756948A73F133053F17A1AA086767B81085`；26.1.2 SHA-256 `19BB1F997FC602C21117EEDDA18423834EFC35D4A4D6DB125F1B53861B731CD7`。详见 `archive/2026-08-26-virtual-elytra-expansion-and-ruin-silhouette.md`。

> **2026-08-26 虚拟鞘翅节点语义修复：** 虚拟鞘翅只在尝试展开时检查灵气护体；展开后的滑翔状态只依据 marker，不因效果变化重复检查。未展开/已收起时才同步检查效果以控制显隐和下一次展开资格，落地清除 marker。26.1.2 修复重复 tick 注入与错误 LivingEntity API 调用；信息存储水晶继续显示方块本地化名称并支持 Mekanism BlockEntity 通用 NBT 保存/粘贴。详见 `archive/2026-08-26-virtual-elytra-node-and-crystal-compat.md`。

> **2026-08-25 信息存储水晶与 26.1.2 迁移完成：** 仙器建筑模式 HUD 已明确为整个相连表面、64 仅为每 tick 批处理预算且不再作为玩法总上限；扳手距离、动态工具、虚拟鞘翅和仙墟渲染的双版本实现均已构建。信息存储水晶现支持双版本 Shift 右键按机器类型保存最新状态、普通右键粘贴、无状态方块/空气 Shift 右键清空、tooltip 显示已保存方块名称，并在最高优先级拦截以避免打开机器 UI；26.1.2 使用 ValueInput/ValueOutput 迁移桥保持配置与插件数据。两代完整 build 均通过，制品已备份并替换 PCL 版本实例及 26.1.2 全局 mods，未启动客户端。详情见 `archive/2026-08-25-immortalstorage-0.1.2-migration-2612.md`。

> **2026-08-25 仙器扳手触及距离同步修复：** 实机未生效的根因是旧实现只在服务端 tick 挂载临时属性，客户端准星仍可能按默认距离选取，远处目标因而不会发送交互包。现将仙器扳手模式的方块/实体触及距离各 +5 改为物品自身的动态 `HAND` 属性修饰器，客户端与服务端通过装备属性生命周期同步；灵器及仙器其他模式不受影响。双版本定向测试与完整 `build` 均通过，新 JAR 已替换 `dist/0.1.2` 与两代 PCL 实例，未启动客户端。1.21.1 SHA-256 `54D5DF824EFCD4113108FC45AB0B5D0B2C471D732B4763D501B5BA45E8E0C23F`（5,506,402 字节），26.1.2 SHA-256 `AB05ABB10ADA23143421DAE56B938A825ACE21FDB078368E98B77A19E6FC6FA7`（5,517,027 字节）。详情见 `archive/2026-08-25-artifact-wrench-reach-sync-fix.md`。

> **2026-08-25 Create“蕴灵”启动崩溃修复：** 用户实机日志确认 `CreateNurturingCompat#initialize` 在模组构造阶段直接写入已冻结的 `create:fan_processing_type` 注册表，导致 1.21.1 启动失败。现改为 NeoForge `DeferredRegister`，在合法注册事件中登记 `immortalstorage:nurturing`；仙炉配方处理、青白粒子与范围生命恢复 5 行为不变。1.21.1 完整门禁为 234 suites / 844 tests，26.1.2 为 233 suites / 834 tests，均零失败；新制品已替换 `dist/0.1.2` 与两代 PCL 实例但未启动客户端。1.21.1 SHA-256 `1CCC2FDA6E34F2E30464E3BA392810BD87940C60BA6BC88343AFEF3193CC6198`（5,505,397 字节），26.1.2 SHA-256 `CD6BE5FFBE22F89AD77ED6F7977569F63CD728CC129C1BFEF1FAD85169229BB2`（5,516,904 字节）。

> **2026-08-25 仙藏 0.1.2 最终闭环并部署：** 在此前完成项上继续修复了灵气护体鞘翅未实际半透明、仙器精准换色可能压缩原材质色阶、GUI 仙墟仍走圆形核心、模组方块 DIG 能力暴露不足、可剪生物准星不切换剪刀等缺口，并新增仙器扳手模式手持时 +5 格实体触及距离（灵器不变）。完整中英手册同步更新。1.21.1 通过 234 suites / 844 tests，26.1.2 通过 233 suites / 834 tests，均 0 failures/errors/skips 且完整 `build` 门禁通过。最终制品已替换两代 PCL 实例但未启动客户端；旧 0.1.1 位于 `archive/pcl2-backups/2026-08-25-immortalstorage-0.1.2-final/`。1.21.1 SHA-256 `ED1735EBA0A5C9EE6C42CDADED18E670A95436F6D458406A9CD39BD6A96A510E`（5,505,162 字节），26.1.2 SHA-256 `50C54815362A6D81D04263F920762C291983A309A364694A40633C8830809409`（5,516,892 字节）。

> **2026-08-25 仙藏 0.1.2 双版本开发制品完成：** 九项规划与追加需求已落地，包括原生末地矿脉、仙锻材料链、仙师度/灵气护体、仙器五模式、迷你仙墟方块核心、完整双语古玉指导书、五项双语成就、Curios 护符槽和 1.21.1 Create“蕴灵”联动。“天上天下，唯我独尊”图标严格使用现有 `immortalstorage:white_day_thunder`（白日惊雷）；“这算作弊吗”使用末影龙刷怪蛋。1.21.1 使用 PCL 实例现成兼容 JAR 离线通过 841 项测试，26.1.2 通过 831 项测试；均通过兼容矩阵、生产边界、版本组成和无 AE2 运行时门禁。制品位于 `dist/0.1.2/`，本轮未部署或启动 PCL2。

> **2026-08-25 0.1.2 十阶段成就图标确认：** “天上天下，唯我独尊”的 10 阶段节点图标固定使用物品 `immortalstorage:white_day_thunder`，即“白日惊雷”；不带“树”字，也不新增或重绘成就图标。

> **2026-08-25 仙师度英文名核验：** 玩家可见英文统一为 `Immortal One's Rule`；不要求在显示文本中保留下划线，`One's` 按英语所有格显示。代码内部标识符继续按对应语言的命名习惯使用。

> **2026-08-25 仙师度资源定稿：** 玉佩正式采用用户附件原图透明内容区域的 16×16 BOX 像素采样，不重绘、不补像素、不换色；英文显示名固定为 `Immortal One's Rule`。所有 0.1.2 换色资源继续以需求指定的原版材质为唯一像素模板，只替换色系并保持尺寸、alpha、轮廓与像素细节。

> **2026-08-25 仙藏 0.1.2 第一阶段实施中：** 已在 1.21.1 规范源码落地仙法遗骸、仙锻合金碎片/合金、三种烧炼、两张无序配方、精准换色资源及末地采集器激活块变更。仙法遗骸按最新要求改用原生 `minecraft:ore` 矿脉：末地外岛、矿脉大小 3、平均每 4 区块尝试一次、高度 0–128；不再采用逐末地石 `1/1000` 扫描。灵气护体共享结算明确依次消耗真元、按 1:64 兑换仙元，二者不足时仅把剩余部分结算为生命伤害。JDK 21 编译及定向测试通过；26.1.2 迁移、矿物池实机确认和后续装备/工具/渲染仍待实施。

> **2026-08-25 仙藏 0.1.2 更新规划：** 已完成九项需求的双版本实施规划，覆盖仙法遗骸与仙锻材料链、世界碎片采集器末地结构、仙师度/灵气护体、仙器五模式强化，以及五类迷你仙墟方块核心渲染。计划按“版本 API 探针 → 矿物材料 → 仙师度 → 仙器 → 渲染 → 双版本发布”分为 18 个可验收任务，并把 26.1.2 鹦鹉螺护甲、伤害绕过补偿、虚拟鞘翅、多装备槽和无限建筑请求列为先行风险探针。规划见 `tasks/plan.md`，执行清单见 `tasks/todo.md`；本轮仅规划，未改功能代码、未升版、未构建或部署。
...
## 0.1.3 仙窍红石接口阈值持久化（2026-08-28）

仙窍红石接口打开界面后会等待服务端配置同步完成，再显示已保存的高、低阈值与正反相状态，避免客户端初始零值覆盖真实配置。高低阈值输入框支持主键盘 Enter 与小键盘 Enter：按下后立即规范化数值、发送配置、保存方块实体 NBT，并立刻重新计算红石输出；关闭界面仍保留提交兜底。

两代完整 `build --offline` 均通过，兼容类路径只读取 PCL `versions` 下对应专属实例的现成 JAR。1.21.1 制品 SHA-256 为 `D489266F98532EA4040CF9956476829AFDFA3D7E1E4BD98505C149F0BEEA5A67`；26.1.2 制品 SHA-256 为 `6C24CB79B8CEE4EA84AF5F2CBAA0F719121EDED0E548929BC67441C1AF6D4250`。两代均已部署到对应 `ImmortalStorage-*` PCL 实例，替换前旧包保存在 `archive/pcl2-backups/2026-08-28-0.1.3-redstone-threshold-enter-persistence/`。本轮未启动客户端。
