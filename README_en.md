...
> **0.1.3 dual-version release (2026-08-28):** Adds the Xianqiao Redstone Interface, categorized machine tags, and a shared server-authoritative three-state redstone work mode. The release also includes the compact persisted threshold UI, corrected Blockbench active/inactive resources, standard block-item transforms, and advanced ruin settings-layout fixes. [Download 0.1.3](https://github.com/positer/ImmortalStorage/releases/tag/0.1.3). This repository and Release contain ImmortalStorage only; no other mod from the surrounding development workspace is included.
