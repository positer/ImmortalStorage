package com.cultivation.cultivation.debug;
...
import com.cultivation.cultivation.network.storage.PersonalStorageItemHandler;
...
public final class CultivationSelfTest {
    ...
    private static void verifyProgressionSmoke(List<String> failures) {
        ...
        expect(failures, com.cultivation.cultivation.dimension.RealmHelper.runtimeForcedRadiusChunks(data) == 3,
    ...
    private static void verifyPersonalRealmKeys(List<String> failures) {
        ...
        var firstKey = CultivationDimensions.personalRealmKey(first);
        var secondKey = CultivationDimensions.personalRealmKey(second);
        expect(failures, firstKey.equals(CultivationDimensions.personalRealmKey(first)), "personal realm key is stable");
        expect(failures, !firstKey.equals(secondKey), "personal realm keys differ per player uuid");
        expect(failures, CultivationDimensions.isPersonalRealmFor(firstKey, first), "personal realm key matches owner");
        expect(failures, !CultivationDimensions.isPersonalRealmFor(firstKey, second), "personal realm key rejects other owner");
        expect(failures, CultivationDimensions.personalRealmOwner(firstKey).orElseThrow().equals(first), "personal realm owner decodes");
        expect(failures, CultivationDimensions.isXianqiaoRealm(firstKey), "personal realm prefix recognized");
        expect(failures, !CultivationDimensions.isXianqiaoRealm(net.minecraft.world.level.Level.OVERWORLD), "overworld is not xianqiao realm");
        expect(failures, !CultivationDimensions.isXianqiaoRealm(CultivationDimensions.XIANQIAO_REALM_LEVEL), "template shared level is not a player realm");
    }
    ...
    private static void verifyClientAssets(List<String> failures) {
        ...
        expect(failures, resourceExists("assets/cultivation/lang/en_us.json"), "client lang en_us");
        for (String id : itemIds) {
            expect(failures, resourceExists("assets/cultivation/models/item/" + id + ".json"), "item model " + id);
            expect(failures, resourceExists("assets/cultivation/textures/item/" + id + ".png"), "item texture " + id);
        }
        for (String id : blockIds) {
            expect(failures, resourceExists("assets/cultivation/blockstates/" + id + ".json"), "blockstate " + id);
            expect(failures, resourceExists("assets/cultivation/models/block/" + id + ".json"), "block model " + id);
            expect(failures, resourceExists("assets/cultivation/models/item/" + id + ".json"), "block item model " + id);
            expect(failures, resourceExists("assets/cultivation/textures/block/" + id + ".png"), "block texture " + id);
        }
        expect(failures, resourceExists("assets/cultivation/textures/reference/image2_cultivation_texture_atlas.png"), "Image2 texture reference");
    }

    private static void verifyMenuLayouts(List<String> failures) {
        expect(failures, com.cultivation.cultivation.api.storage.terminal.TerminalViewport.COLUMNS == 9,
                "terminal uses nine storage columns");
        expect(failures, com.cultivation.cultivation.api.storage.terminal.TerminalViewport.MIN_ROWS == 2
                        && com.cultivation.cultivation.api.storage.terminal.TerminalViewport.DEFAULT_ROWS == 5
                        && com.cultivation.cultivation.api.storage.terminal.TerminalViewport.MAX_ROWS == 12,
                "terminal row limits are 2..12 with five-row default");
        expect(failures, com.cultivation.cultivation.menu.custom.KongqiaoMenu.VISIBLE_STORAGE_SLOTS == 108
                        && com.cultivation.cultivation.menu.custom.XianqiaoStorageMenu.BUFFERED_STORAGE_SLOTS == 216,
                "Kongqiao reserves twelve rows and Xianqiao preloads a double twenty-four-row window");
        expect(failures, com.cultivation.cultivation.menu.custom.KongqiaoMenu.CRAFT_START == 108
                        && com.cultivation.cultivation.menu.custom.XianqiaoStorageMenu.CRAFT_START == 216,
    ...
    private static void verifyTerminalContracts(List<String> failures) {
        var viewport = new com.cultivation.cultivation.api.storage.terminal.TerminalViewport(99, 99, 99, 7, -1);
        ...
        expect(failures, com.cultivation.cultivation.api.storage.terminal.TerminalViewport.clampBaseRow(20, 5, 12) == 7,
                "terminal viewport reaches the final partial page");
        expect(failures, Math.min(com.cultivation.cultivation.api.storage.terminal.TerminalViewport.MAX_ROWS,
                        com.cultivation.cultivation.api.storage.terminal.TerminalViewport.DEFAULT_ROWS + 1) == 6,
        ...
        var catalog = new com.cultivation.cultivation.api.storage.terminal.TerminalEntryCatalog();
        ...
        List<com.cultivation.cultivation.api.storage.terminal.TerminalEntry> entries =
                catalog.entries(com.cultivation.cultivation.api.storage.terminal.TerminalQuery.DEFAULT);
        ...
        long ironId = entries.stream().filter(entry -> entry.displayStack().is(Items.IRON_INGOT) && entry.amount() == 80)
                .findFirst().map(com.cultivation.cultivation.api.storage.terminal.TerminalEntry::entryId).orElse(0L);
        ...
        var modQuery = new com.cultivation.cultivation.api.storage.terminal.TerminalQuery("@minecraft",
                com.cultivation.cultivation.api.storage.terminal.TerminalQuery.SortOrder.NAME,
                com.cultivation.cultivation.api.storage.terminal.TerminalQuery.SortDirection.ASCENDING);
        expect(failures, catalog.entries(modQuery).size() == 3, "terminal namespace query filters entries");
        expect(failures, new com.cultivation.cultivation.api.storage.terminal.TerminalQuery("x".repeat(256), null, null)
                        .text().length() == com.cultivation.cultivation.api.storage.terminal.TerminalQuery.MAX_SEARCH_LENGTH,
                "terminal query bounds untrusted search text");

        CultivationPlayerData kongqiao = new CultivationPlayerData();
        kongqiao.setStage(1);
        for (int i = 0; i < kongqiao.getKongqiaoMaxSlots(); i++) {
            kongqiao.setKongqiaoSlot(i, new ItemStack(Items.COBBLESTONE, 64));
        }
        ItemStack kongqiaoLeftover = com.cultivation.cultivation.menu.custom.TerminalMenuSupportTestAccess
    ...
    private static void verifySourceVeinRegistrations(List<String> failures) {
        ...
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.outputCostPerTick(VeinKind.STONE, 64) == 1,
                "stone source cost at 64 flux");
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.outputCostPerTick(VeinKind.STONE, 65) == 2,
                "stone source cost rounds up over 64 flux");
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.outputCostPerTick(VeinKind.NETHER_STAR, 1) == 8,
                "nether star source cost applies per-output batch");
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.outputCostPerTick(VeinKind.DRAGON_EGG, 1) == 64,
        ...
        cobble.setSideMode(Direction.NORTH, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.SourceSideMode.DISABLED);
        ...
        cobble.setSideMode(Direction.EAST, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.SourceSideMode.PUSH);
        ...
        expect(failures, cobble.cycleSideMode(Direction.EAST)
                        == com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.SourceSideMode.DISABLED,
    ...
    private static void verifyPublicCompatibilityApi(List<String> failures) {
        ...
        var waterEndpoint = com.cultivation.cultivation.api.source.SourceApi.resolve(sourceVein(ModBlocks.WATER_VEIN.get(), VeinKind.WATER));
        ...
        var stoneEndpoint = com.cultivation.cultivation.api.source.SourceApi.resolve(sourceVein(ModBlocks.STONE_VEIN.get(), VeinKind.STONE));
        ...
        var endpoint = new com.cultivation.cultivation.network.storage.PersonalStorageNetwork.Endpoint(
    ...
    private static void verifySourceOwnershipPolicy(List<String> failures) {
        ...
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.canClaim(null, actor, 0, 0, false),
                "source ownership unowned claim is allowed");
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.canClaim(owner, owner, 0, 0, false),
                "source ownership owner can reopen");
        expect(failures, !com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.canClaim(owner, actor, 0, 0, false),
                "source ownership other claim obeys config deny");
        expect(failures, com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.canClaim(owner, actor, 6, 6, true),
                "source ownership other claim obeys config allow");
        expect(failures, !com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.canClaim(owner, actor, 5, 6, true),
    ...
    private static void expectSourceOutput(List<String> failures, VeinKind kind, net.minecraft.world.item.Item item, String label) {
        ItemStack stack = com.cultivation.cultivation.block.entity.SourceVeinBlockEntity.kindToStack(kind, 3);
        expect(failures, !stack.isEmpty() && stack.is(item) && stack.getCount() == 3, label);
    }

    private static com.cultivation.cultivation.block.entity.SourceVeinBlockEntity sourceVein(Block registeredBlock, VeinKind kind) {
        SourceVeinBlock block = (SourceVeinBlock) registeredBlock;
        BlockState state = block.defaultBlockState();
        return new com.cultivation.cultivation.block.entity.SourceVeinBlockEntity(BlockPos.ZERO, state, kind);
    }

    private static boolean resourceExists(String path) {
        return CultivationSelfTest.class.getClassLoader().getResource(path) != null;
    }
    ...
    public record Result(List<String> failures) {
        ...
        public String summary() {
            return passed()
                    ? "Cultivation selftest PASS: stages=11, yuan math, time scale, tribulation, staff mode, progression smoke, personal realm keys, client assets+textures, terminal contracts+aggregation, source vein registrations, source ownership policy, source side io, source read-only io, manager bidirectional io, public compat api"
                    : "Cultivation selftest FAIL: " + String.join("; ", failures);
        }
    }

    private CultivationSelfTest() {}
}
