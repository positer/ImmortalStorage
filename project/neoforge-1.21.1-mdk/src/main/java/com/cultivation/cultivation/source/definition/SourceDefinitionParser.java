...
public final class SourceDefinitionParser {
    ...
    public static SourceDefinition parse(ResourceLocation source, JsonObject json) {
        ...
        ResourceLocation output = id(source,
        ...
        List<ResourceLocation> aliases = parseAliases(source, json.get("aliases"));
        try {
    ...
    private static long nonNegativeLong(ResourceLocation source, JsonObject json, String field, long fallback) {
    ...
    private static long positiveLong(ResourceLocation source, JsonObject json, String field, long fallback) {
        long value = json.has(field) ? integralLong(source, json.get(field), field) : fallback;
        if (value <= 0L) throw error(source, field + " must be positive");
        return value;
    }

    private static long integralLong(ResourceLocation source, JsonElement element, String field) {
    ...
    private static List<ResourceLocation> parseAliases(ResourceLocation source, JsonElement element) {
        ...
        for (JsonElement alias : array) {
            ...
            ResourceLocation parsed = id(source, alias.getAsString(), "aliases");
            if (!result.add(parsed)) throw error(source, "duplicate alias " + parsed);
        }
        return List.copyOf(result);
    }

    private static ResourceLocation id(ResourceLocation source, String raw, String field) {
        ResourceLocation parsed = ResourceLocation.tryParse(raw);
        if (parsed == null) throw error(source, field + " is not a valid resource location: " + raw);
        return parsed;
    }

    private static IllegalArgumentException error(ResourceLocation source, String message) {
