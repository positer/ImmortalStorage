...
public final class SourceVeinManagerInventory extends ItemStackHandler {
    ...
    public void reconcileLoadedMembers() {
        ...
        for (int slot = 0; slot < getSlots(); slot++) {
            ResourceLocation id = sourceDefinitionId(stacks.get(slot));
            if (id != null && !seen.add(id)) duplicates++;
        }
        if (duplicates != inactiveDuplicateCount) {
            inactiveDuplicateCount = duplicates;
            if (duplicates > 0) {
                CultivationMod.LOG.warn("Source Vein Manager retained {} duplicate legacy member(s); "
    ...
    public static ResourceLocation sourceDefinitionId(ItemStack stack) {
        if (stack == null || stack.isEmpty()) return null;
        VeinKind kind = sourceKind(stack);
        ResourceLocation id = kind == null ? null : SourceDefinitions.legacyId(kind);
        if (ModDataComponents.SOURCE_DEFINITION_ID.isBound()) {
            ResourceLocation componentId = stack.get(ModDataComponents.SOURCE_DEFINITION_ID.get());
            if (componentId != null) id = componentId;
        }
        if (id == null && stack != null && !stack.isEmpty()) {
            CompoundTag tag = stack.getOrDefault(DataComponents.BLOCK_ENTITY_DATA, CustomData.EMPTY).copyTag();
            if (tag.contains("SourceDefinitionId", net.minecraft.nbt.Tag.TAG_STRING)) {
                id = ResourceLocation.tryParse(tag.getString("SourceDefinitionId"));
            }
        }
    ...
    private boolean duplicatesExistingDefinition(int targetSlot, ItemStack candidate) {
        ResourceLocation candidateId = sourceDefinitionId(candidate);
    ...
    public static long cachedUnits(ItemStack stack) {
        ...
        return Math.max(0L, data.copyTag().getLong(CACHE_TAG));
    }
