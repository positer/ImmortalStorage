 ...
 * and disk identities. Keeping the base item API-only makes a Cultivation
...
public final class XianqiaoExchangeCellItem extends Item {
    ...
    private static final String DISK_TAG = "cultivationExchangeDisk";
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (data.getStage() < 6) {
            player.displayClientMessage(Component.translatable(
                    "message.cultivation.xianqiao_exchange_cell.requires_xianqiao"), true);
            return InteractionResultHolder.fail(stack);
        }
        bindUnbound(stack, player.getUUID(), UUID.randomUUID(), playerName(player));
        if (!isBoundToOwner(stack, player.getUUID())) {
            player.displayClientMessage(Component.translatable(
                    "message.cultivation.xianqiao_exchange_cell.wrong_owner"), true);
            return InteractionResultHolder.fail(stack);
        }
        refreshOwnerName(stack, player.getUUID(), playerName(player));
        player.displayClientMessage(Component.translatable(
                "message.cultivation.xianqiao_exchange_cell.bound"), true);
    ...
    public void appendHoverText(
        ...
        if (owner.isEmpty() || disk.isEmpty()) {
            lines.add(Component.translatable(
                    "tooltip.cultivation.xianqiao_exchange_cell.unbound")
                    .withStyle(ChatFormatting.GRAY));
            return;
        }
        Object displayOwner = ownerName(stack).<Object>map(name -> name).orElse(owner.get());
        lines.add(Component.translatable(
                "tooltip.cultivation.xianqiao_exchange_cell.owner", displayOwner)
                .withStyle(ChatFormatting.AQUA));
        lines.add(Component.translatable(
                "tooltip.cultivation.xianqiao_exchange_cell.disk", disk.get())
