...
public final class CultivationKeybinds {
    public static final KeyMapping OPEN_STORAGE = new KeyMapping(
            "key.cultivation.open_storage", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_B, "key.categories.cultivation");
    ...
    public static void init(IEventBus modBus, IEventBus forgeBus) {
        ...
        forgeBus.addListener(CultivationKeybinds::onClientTick);
        forgeBus.addListener(CultivationKeybinds::onScreenKeyPressed);
        forgeBus.addListener(CultivationKeybinds::onMouseScroll);
    }
    ...
    public static void onClientTick(ClientTickEvent.Post e) {
        ...
        if (TIME_FLOW.consumeClick()) {
            if (d.getStage() < 7) {
                ...
                p.displayClientMessage(Component.translatable("container.cultivation.terminal.time_open_realm")
    ...
    public static void onScreenKeyPressed(ScreenEvent.KeyPressed.Pre event) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null || event.getScreen() instanceof com.cultivation.cultivation.client.screen.TerminalScreenAccess) {
            return;
        }
        ...
        requestStorageOpen(mc.player, CultivationPlayerData.get(mc.player), event.getScreen());
        event.setCanceled(true);
    }

    private static void requestStorageOpen(Player player, CultivationPlayerData data,
                                           net.minecraft.client.gui.screens.Screen returnScreen) {
        if (data.getStage() < 1) {
            player.displayClientMessage(Component.translatable("message.cultivation.not_cultivating")
                    .withStyle(ChatFormatting.RED), true);
            return;
        }
        com.cultivation.cultivation.client.screen.TerminalReturnNavigation.arm(returnScreen);
        if (data.getStage() >= 6) {
