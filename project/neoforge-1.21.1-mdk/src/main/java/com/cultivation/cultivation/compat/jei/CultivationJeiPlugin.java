...
import com.cultivation.cultivation.client.screen.TerminalScreenAccess;
import com.cultivation.cultivation.client.screen.TerminalFluidScreenAccess;
...
public final class CultivationJeiPlugin implements IModPlugin {
    private static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(CultivationMod.MODID, "terminal_jei");
    ...
    public static void setSearchText(String text) {
        if (runtime != null && CultivationClientConfig.SYNC_RECIPE_VIEWER_SEARCH.get()) {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        if (level != null) registration.addRecipes(ImmortalFurnaceJeiCategory.TYPE,
                level.getRecipeManager().getAllRecipesFor(
                        com.cultivation.cultivation.recipe.ModRecipes.IMMORTAL_FURNACE_TYPE.get()));
    }

    @Override
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        registration.addRecipeCatalyst(com.cultivation.cultivation.block.ModBlocks.IMMORTAL_FURNACE.get(),
    ...
    private static final class TerminalGuiHandler<S extends net.minecraft.client.gui.screens.inventory.AbstractContainerScreen<?>>
        ...
        public List<Rect2i> getGuiExtraAreas(S screen) {
            return ((TerminalScreenAccess) screen).cultivation$getExtraAreas();
        }
        ...
        public Optional<? extends mezz.jei.api.runtime.IClickableIngredient<?>> getClickableIngredientUnderMouse(
            ...
            if (screen instanceof TerminalFluidScreenAccess fluids) {
                Optional<TerminalFluidScreenAccess.FluidHover> hover =
                        fluids.cultivation$getFluidAt(mouseX, mouseY);
                if (hover.isPresent()) {
            ...
            Slot slot = ((TerminalScreenAccess) screen).cultivation$getSlotAt(mouseX, mouseY);
            if (slot == null || !slot.hasItem()) {
                return Optional.empty();
            }
            return factory.createBuilder(slot.getItem()).buildWithArea(
                    ((TerminalScreenAccess) screen).cultivation$getSlotBounds(slot));
        }
    }
        ...
        public IRecipeTransferError transferRecipe(M menu, RecipeHolder<CraftingRecipe> recipe,
            ...
            if (!menu.isCraftingUnlocked() || !menu.isCraftingVisible()) {
                return helper.createUserErrorWithTooltip(
                        Component.translatable("container.cultivation.terminal.recipe_transfer_locked"));
            }
            ...
            if (transferableSets <= 0) {
                return helper.createUserErrorForMissingSlots(
                        Component.translatable("container.cultivation.terminal.recipe_transfer_missing"),
                        recipeSlots.getSlotViews(RecipeIngredientRole.INPUT));
            }
            if (doTransfer) {
                PacketDistributor.sendToServer(new com.cultivation.cultivation.network.ModPayloads.TransferTerminalRecipe(
