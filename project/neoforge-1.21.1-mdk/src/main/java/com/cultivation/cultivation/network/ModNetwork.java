package com.cultivation.cultivation.network;
...
import com.cultivation.cultivation.block.entity.XianqiaoInterfaceLimits;
import com.cultivation.cultivation.block.entity.XianqiaoInterfaceInventory;
import com.cultivation.cultivation.api.storage.terminal.TerminalRecipeAvailability;
import com.cultivation.cultivation.menu.custom.KongqiaoMenu;
import com.cultivation.cultivation.menu.custom.SourceVeinMenu;
import com.cultivation.cultivation.menu.custom.SourceFluxValue;
...
import com.cultivation.cultivation.menu.provider.KongqiaoProvider;
import com.cultivation.cultivation.menu.provider.XianqiaoStorageProvider;
...
public final class ModNetwork {
    ...
    private static void handleOpenKongqiao(ModPayloads.OpenKongqiao m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (d.getStage() < 1 || d.getStage() >= 6) {
                sp.displayClientMessage(Component.literal("Kongqiao is available only at stages 1-5"), true);
                return;
            }
    ...
    private static void handleOpenXianqiao(ModPayloads.OpenXianqiaoStorage m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            CultivationPlayerData d = CultivationPlayerData.get(sp);
            if (d.getStage() < 6) {
                sp.displayClientMessage(Component.literal("Ascension stage required"), true);
                return;
            }
    ...
    private static void handleTriggerTribulation(ModPayloads.TriggerTribulation m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            CultivationPlayerData d = CultivationPlayerData.get(sp);
            int st = d.getStage();
            if (!com.cultivation.cultivation.progression.TribulationPolicy.canStart(
                    st, com.cultivation.cultivation.progression.TribulationPolicy.configuredMaximumStage())) {
                sp.displayClientMessage(Component.literal("Current stage cannot start tribulation"), true);
                return;
            }
            if (d.isTribulationActive()) {
                sp.displayClientMessage(Component.literal("Tribulation already active"), true);
                return;
            }
            ...
            if (com.cultivation.cultivation.event.TribulationHelper.start(sp)) {
                sp.displayClientMessage(Component.literal("Tribulation begins. Defeat the bound target."), true);
            } else {
                sp.displayClientMessage(Component.literal("Tribulation target could not be created"), true);
            }
        });
    }
    ...
    private static void handleTimeFlow(ModPayloads.TimeFlow m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            int st = d.getStage();
            if (st < 7) {
                sp.displayClientMessage(Component.literal("Stage 7+ required to adjust time flow"), true);
                return;
            }

            int nextPermille = com.cultivation.cultivation.dimension.RealmTimeScalePolicy.stepPermille(
                    st, d.getRealmTimeRatePermille(), m.delta());
            d.setRealmTimeRatePermille(nextPermille);
            com.cultivation.cultivation.dimension.RealmHelper.refreshRealmTickRate(sp);
            sp.displayClientMessage(Component.literal("Time flow: " + d.getTimeScale() + "x"), true);
        });
    }
    ...
    private static void handleSetTerminalQuery(ModPayloads.SetTerminalQuery payload, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            var query = new com.cultivation.cultivation.api.storage.terminal.TerminalQuery(
                    payload.text(),
                    com.cultivation.cultivation.api.storage.terminal.TerminalQuery.SortOrder.byId(payload.sortOrder()),
                    com.cultivation.cultivation.api.storage.terminal.TerminalQuery.SortDirection.byId(payload.sortDirection()));
    ...
    private static void handleTerminalEntryAction(ModPayloads.TerminalEntryAction payload, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (player == null || !(player.containerMenu instanceof XianqiaoStorageMenu menu)
                    || !hasLiveXianqiaoMenu(player, menu)) return;
            var action = com.cultivation.cultivation.api.storage.terminal.TerminalAction.byId(payload.action());
    ...
    private static void handleTransferTerminalRecipe(ModPayloads.TransferTerminalRecipe payload, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (player == null || player.containerMenu.containerId != payload.containerId()
                    || staleXianqiao
                    || !(player.containerMenu instanceof com.cultivation.cultivation.api.storage.terminal.CraftingTransferTarget target)
                    || !(player.containerMenu instanceof com.cultivation.cultivation.api.storage.terminal.StorageTerminalView terminal)
            ...
            List<com.cultivation.cultivation.api.storage.terminal.CraftingTransferTarget.TransferIngredient> storage =
            ...
            if (!hasRecipeIngredients(player, target, storage, recipe.getIngredients())) return;
    ...
    private static boolean hasRecipeIngredients(ServerPlayer player,
                                                com.cultivation.cultivation.api.storage.terminal.CraftingTransferTarget target,
                                                List<com.cultivation.cultivation.api.storage.terminal.CraftingTransferTarget.TransferIngredient> storage,
    ...
    public static void sendRecipeSources(ServerPlayer player, net.minecraft.world.inventory.AbstractContainerMenu menu,
                                         List<com.cultivation.cultivation.api.storage.terminal.CraftingTransferTarget.TransferIngredient> sources,
                                         long revision) {
    ...
    private static void handleToggleRealm(ModPayloads.ToggleRealm m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ServerPlayer sp = serverPlayer(ctx);
            if (sp == null) return;
            if (com.cultivation.cultivation.dimension.CultivationDimensions.isPersonalRealmFor(sp.level().dimension(), sp.getUUID())) {
                com.cultivation.cultivation.dimension.RealmHelper.exitRealm(sp);
            } else {
                com.cultivation.cultivation.dimension.RealmHelper.enterRealm(sp);
            }
        });
    }
    ...
    private static void handleCycleStaffMode(ModPayloads.CycleStaffMode m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (stack.getItem() instanceof com.cultivation.cultivation.item.custom.SpiritStaffItem) {
                com.cultivation.cultivation.item.custom.SpiritStaffItem.cycleMode(sp, m.delta());
            }
        });
    }
    ...
    private static void handleSetSourceSideMode(ModPayloads.SetSourceSideMode m, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (!source.isOwnedBy(sp.getUUID()) && !sp.hasPermissions(2)) {
                sp.displayClientMessage(Component.translatable("message.cultivation.source_vein.configure.denied"), true);
                return;
            }
    ...
    private static void handleSpiritStaffBuildRemoval(
        ...
        ctx.enqueueWork(() -> {
            ...
            var result = com.cultivation.cultivation.item.custom.SpiritStaffBuildExecutor.removeLayer(
                    player, net.minecraft.world.InteractionHand.values()[request.hand()], clicked,
                    Direction.values()[request.face()],
                    com.cultivation.cultivation.config.CultivationConfig.SPIRIT_STAFF_BUILD_LIMIT.get());
    ...
    private static SourceVeinBlockEntity validateOpenOwnedSource(ServerPlayer player, int containerId, BlockPos pos) {
        ...
        if (!source.isOwnedBy(player.getUUID())) {
            player.displayClientMessage(Component.translatable("message.cultivation.source_vein.configure.denied"), true);
            return null;
        }
