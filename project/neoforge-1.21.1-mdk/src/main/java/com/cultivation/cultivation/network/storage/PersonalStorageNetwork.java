 ...
 *
 * The backing data still lives on CultivationPlayerData; this class provides a
...
public final class PersonalStorageNetwork {
    ...
    public static @Nullable Endpoint resolve(MinecraftServer server, @Nullable UUID owner, @Nullable Runnable onChanged) {
        ...
        BooleanSupplier accessAllowed = () -> {
            ...
            return currentPlayer != null
                    && CultivationPlayerData.get(currentPlayer) == data
                    && data.getStage() >= 1;
        };
    ...
    public static @Nullable Endpoint resolveXianqiao(
        ...
        boolean includeFluids = data.getStage() >= CultivationPlayerData.XIANQIAO_FLUID_UNLOCK_STAGE;
    ...
    public static @Nullable Endpoint resolveInOwnerRealm(
        ...
        if (!CultivationDimensions.isPersonalRealmFor(realm.dimension(), owner)) return null;
        ...
        BooleanSupplier accessAllowed = () -> {
            ...
            return currentPlayer != null
                    ...
                    && CultivationDimensions.isPersonalRealmFor(realm.dimension(), owner);
        };
        boolean includeFluids = data.getStage() >= CultivationPlayerData.XIANQIAO_FLUID_UNLOCK_STAGE;
        return new Endpoint(owner, data, player.registryAccess(), onChanged,
    ...
    private static @Nullable Endpoint resolveWithFluids(
        ...
        if (player == null) return null;
        CultivationPlayerData data = CultivationPlayerData.get(player);
        if (data.getStage() < CultivationPlayerData.XIANQIAO_FLUID_UNLOCK_STAGE) return null;
        BooleanSupplier accessAllowed = () -> {
            ServerPlayer currentPlayer = server.getPlayerList().getPlayer(owner);
            return currentPlayer != null
                    && CultivationPlayerData.get(currentPlayer) == data
                    && data.getStage() >= CultivationPlayerData.XIANQIAO_FLUID_UNLOCK_STAGE
    ...
    public static final class Endpoint implements PersonalStorageEndpoint {
        ...
        private final CultivationPlayerData data;
        ...
        public Endpoint(UUID owner, CultivationPlayerData data, HolderLookup.Provider registryAccess, @Nullable Runnable onChanged) {
            this(owner, data, registryAccess, onChanged, true);
        }

        public Endpoint(UUID owner, CultivationPlayerData data, HolderLookup.Provider registryAccess, @Nullable Runnable onChanged, boolean online) {
            this(owner, data, registryAccess, onChanged, online,
                    () -> data != null && data.getStage() >= 1, false, null);
        }

        private Endpoint(UUID owner, CultivationPlayerData data, HolderLookup.Provider registryAccess,
        ...
        public CultivationPlayerData data() {
            return data;
        }
        ...
        public ItemStack extract(ItemStack template, int amount, boolean simulate) {
            ...
            long extracted = itemStorage.extract(
                    com.cultivation.cultivation.api.storage.terminal.TerminalEntryKey.of(template), request,
                    simulate
                            ? com.cultivation.cultivation.api.storage.terminal.TerminalStorageAction.SIMULATE
                            : com.cultivation.cultivation.api.storage.terminal.TerminalStorageAction.EXECUTE);
