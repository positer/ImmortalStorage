package com.immortalstorage.immortalstorage.compat.buildinggadgets;
...
public final class BuildingGadgetsStorageBridge {
    ...
    public static int countAvailable(Player player, ItemStack template) {
        ...
        long count = availableItems(ImmortalStoragePlayerData.get(player))
                .getOrDefault(TerminalEntryKey.of(template), 0L);
        return (int) Math.min(Integer.MAX_VALUE, Math.max(0L, count));
    }

    private static Map<TerminalEntryKey, Long> availableItems(ImmortalStoragePlayerData data) {
