...
public final class ClientNetworkHandlers {
    ...
    private static void handleSpiritStaffBuildPreview(
        ...
        ctx.enqueueWork(() -> com.immortalstorage.immortalstorage.client.render.SpiritStaffBuildPreview
    ...
    private static void handleTerminalRecipeSources(ModPayloads.TerminalRecipeSources chunk, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (minecraft.player == null || minecraft.player.containerMenu.containerId != chunk.containerId()
                    || !(minecraft.player.containerMenu instanceof com.immortalstorage.immortalstorage.api.storage.terminal.CraftingTransferTarget target)) {
                return;
            }
            java.util.List<com.immortalstorage.immortalstorage.api.storage.terminal.CraftingTransferTarget.TransferIngredient> entries =
                    chunk.entries().stream()
                            .map(entry -> new com.immortalstorage.immortalstorage.api.storage.terminal.CraftingTransferTarget.TransferIngredient(
    ...
    public static void handleTerminalViewSnapshot(ModPayloads.TerminalViewSnapshot snapshot, IPayloadContext ctx) {
        ctx.enqueueWork(() -> {
            ...
            if (minecraft.player == null
                    || !(minecraft.player.containerMenu instanceof com.immortalstorage.immortalstorage.menu.custom.XianqiaoStorageMenu menu)
                    || menu.containerId != snapshot.containerId()) {
                return;
            }
            java.util.List<com.immortalstorage.immortalstorage.api.storage.terminal.TerminalEntry> entries = new java.util.ArrayList<>();
            for (ModPayloads.TerminalViewSnapshot.Entry entry : snapshot.entries()) {
                entries.add(new com.immortalstorage.immortalstorage.api.storage.terminal.TerminalEntry(
    ...
    public static void handleTerminalFluidViewSnapshot(ModPayloads.TerminalFluidViewSnapshot snapshot,
        ...
        ctx.enqueueWork(() -> {
            ...
            java.util.List<com.immortalstorage.immortalstorage.api.storage.terminal.TerminalFluidEntry> entries =
                    new java.util.ArrayList<>();
            for (ModPayloads.TerminalFluidViewSnapshot.Entry entry : snapshot.entries()) {
                entries.add(new com.immortalstorage.immortalstorage.api.storage.terminal.TerminalFluidEntry(
    ...
    public static void handleTerminalExternalViewSnapshot(
        ...
        ctx.enqueueWork(() -> {
            ...
            java.util.List<com.immortalstorage.immortalstorage.api.storage.terminal.TerminalExternalResourceEntry> entries =
                    snapshot.entries().stream().map(entry ->
                            new com.immortalstorage.immortalstorage.api.storage.terminal.TerminalExternalResourceEntry(
                                    entry.entryId(), new com.immortalstorage.core.resource.ResourceChannelKey(
