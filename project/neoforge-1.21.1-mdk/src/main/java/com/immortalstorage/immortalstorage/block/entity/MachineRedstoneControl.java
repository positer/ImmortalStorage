...
public final class MachineRedstoneControl {
    ...
    public static RedstoneWorkMode mode(BlockEntity be) { if (!be.getPersistentData().contains(KEY)) return RedstoneWorkMode.IGNORE; int id = be.getPersistentData().getInt(KEY); return RedstoneWorkMode.values()[Math.max(0, Math.min(RedstoneWorkMode.values().length - 1, id))]; }
