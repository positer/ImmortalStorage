...
/**
 * Stabilized ruin frame whose central sphere is replaced by two smaller
 * counter-rotating opposite-state spheres (one black, one white). Each side
 * with preview enabled draws its selection box.
 */
public final class EntangledStabilizedMiniatureImmortalRuinRenderer implements BlockEntityRenderer<EntangledStabilizedMiniatureImmortalRuinBlockEntity> {
    private static final float ORBIT_RADIUS = 0.16F;
    private static final float BASE_SCALE = 0.15F;

    public EntangledStabilizedMiniatureImmortalRuinRenderer(BlockEntityRendererProvider.Context context) {}

    @Override
    public void render(EntangledStabilizedMiniatureImmortalRuinBlockEntity entity, float partialTick, PoseStack poses,
        ...
        poses.popPose();
        RuinLinkRenderer.render(poses, buffers, entity.getBlockPos(), null);
    ...
    private void renderPreview(PoseStack poses, MultiBufferSource buffers,
                               EntangledStabilizedMiniatureImmortalRuinBlockEntity.Side side,
                               int faceMask) {
    ...
    @Override public boolean shouldRenderOffScreen(EntangledStabilizedMiniatureImmortalRuinBlockEntity entity) { return true; }
