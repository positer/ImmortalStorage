...
import com.immortalstorage.immortalstorage.block.entity.EntangledStabilizedMiniatureImmortalRuinBlockEntity;
...
/**
 * Merged ruin storage (54 slots) with two independent stabilized-style settings
 * groups. Button ids below 100 target the normal side, ids 100-199 the reversed
 * side; within a side the local id follows the stabilized layout (0-11 range
 * steppers, 12/13 preview/enabled, 14/15 frequency). Numeric edits are applied
 * through the side-aware value setter.
 */
public final class EntangledMiniatureRuinMenu extends AbstractContainerMenu implements SideFilterMenu, MachineRedstoneMenu {
    private static final int SIDE_SPAN = 100;
    private static final int PER_SIDE = 10;
    private final Container container;
    private final ContainerData data;
    private final EntangledStabilizedMiniatureImmortalRuinBlockEntity blockEntity;
    ...
    public EntangledMiniatureRuinMenu(int id, Inventory inventory, FriendlyByteBuf buffer) {
        this(id, inventory, new SimpleContainer(55), new SimpleContainerData(20), null,
                buffer == null ? net.minecraft.core.BlockPos.ZERO : buffer.readBlockPos());
    }

    public EntangledMiniatureRuinMenu(int id, Inventory inventory, Container container, ContainerData data) {
        this(id, inventory, container, data,
                container instanceof EntangledStabilizedMiniatureImmortalRuinBlockEntity ruin ? ruin : null,
                container instanceof EntangledStabilizedMiniatureImmortalRuinBlockEntity ruin ? ruin.getBlockPos() : net.minecraft.core.BlockPos.ZERO);
    }

    private EntangledMiniatureRuinMenu(int id, Inventory inventory, Container container, ContainerData data,
                                       EntangledStabilizedMiniatureImmortalRuinBlockEntity blockEntity,
                                       net.minecraft.core.BlockPos blockPos) {
        super(ModMenus.ENTANGLED_MINIATURE_IMMORTAL_RUIN.get(), id);
    ...
    public EntangledStabilizedMiniatureImmortalRuinBlockEntity blockEntity() { return blockEntity; }
    ...
    public void setAuthoritativeValue(int side, int index, int value) {
        if (blockEntity != null && index >= 0 && index < PER_SIDE) blockEntity.setMenuValue(side, index, value);
    }
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        if (local >= 20 && local <= 25) {
            int bit = 1 << (local - 20);
            int current = data.get(base + 9);
            data.set(base + 9, (current & bit) == 0 ? current | bit : current & ~bit);
            return true;
        }
        return false;
    }
