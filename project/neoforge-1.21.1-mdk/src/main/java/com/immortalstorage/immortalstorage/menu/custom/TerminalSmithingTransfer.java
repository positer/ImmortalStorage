...
final class TerminalSmithingTransfer {
    static boolean place(ServerPlayer player, EmbeddedSmithingBackend backend,
        ...
        for (ItemStack stack : player.getInventory().items) if (!stack.isEmpty()) available.add(new Available(stack, stack.getCount()));
        ...
        ItemStack template = reserve(available, value::isTemplateIngredient);
        ItemStack base = reserve(available, value::isBaseIngredient);
        ItemStack addition = reserve(available, value::isAdditionIngredient);
