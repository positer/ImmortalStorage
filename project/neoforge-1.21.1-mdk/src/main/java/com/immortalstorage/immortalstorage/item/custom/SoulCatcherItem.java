...
public final class SoulCatcherItem extends Item {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target,
        ...
        if (!target.save(entityTag)) return InteractionResult.FAIL;
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        CompoundTag root = custom(stack);
        CompoundTag entityTag = root.getCompound(ENTITY_TAG);
        Vec3 spawn = context.getClickLocation().add(Vec3.atLowerCornerOf(context.getClickedFace().getNormal())
                .scale(0.55D));
        Entity entity = EntityType.loadEntityRecursive(entityTag, level, loaded -> {
            loaded.moveTo(spawn.x, spawn.y, spawn.z, player.getYRot(), 0.0F);
            return loaded;
        });
    ...
    public static boolean hasEntity(ItemStack stack) {
        return custom(stack).contains(ENTITY_TAG, CompoundTag.TAG_COMPOUND);
    }

    public static CompoundTag containedEntity(ItemStack stack) {
        return custom(stack).getCompound(ENTITY_TAG).copy();
    }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context,
        ...
        tooltip.add(Component.translatable("tooltip.immortalstorage.soul_catcher.contained",
                tag.contains(NAME_TAG) ? tag.getString(NAME_TAG)
