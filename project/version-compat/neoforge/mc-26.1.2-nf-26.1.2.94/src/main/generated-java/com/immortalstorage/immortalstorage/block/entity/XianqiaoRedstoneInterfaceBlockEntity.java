...
public final class XianqiaoRedstoneInterfaceBlockEntity extends XianqiaoInterfaceBlockEntity implements MenuProvider {
    ...
    @Override protected void saveAdditionalLegacy(CompoundTag tag, HolderLookup.Provider registries) { super.saveAdditionalLegacy(tag, registries); tag.putLong("LowThreshold", lowThreshold); tag.putLong("HighThreshold", highThreshold); tag.putBoolean("Inverted", inverted); }
    @Override protected void loadAdditionalLegacy(CompoundTag tag, HolderLookup.Provider registries) { super.loadAdditionalLegacy(tag, registries); lowThreshold = Math.max(0, tag.getLongOr("LowThreshold", 0L)); highThreshold = Math.max(lowThreshold, tag.getLongOr("HighThreshold", 0L)); inverted = tag.getBooleanOr("Inverted", false); }
}
