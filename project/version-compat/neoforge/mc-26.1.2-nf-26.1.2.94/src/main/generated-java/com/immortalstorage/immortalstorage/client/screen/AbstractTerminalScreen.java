...
abstract class AbstractTerminalScreen<M extends AbstractContainerMenu> extends com.immortalstorage.immortalstorage.compat.mc2612.CompatAbstractContainerScreen<M>
    ...
    protected AbstractTerminalScreen(M menu, Inventory inventory, Component title) {
        super(menu, inventory, title, TerminalLayout.WIDTH, TerminalLayout.imageHeight(TerminalLayout.DEFAULT_ROWS, false));
    ...
    protected void init() {
        ...
        this.searchBox = new EditBox(this.font,
                ...
                Component.translatable("container.immortalstorage.terminal.search"));
        this.searchBox.setBordered(false);
        this.searchBox.setMaxLength(64);
        this.searchBox.setHint(Component.translatable("container.immortalstorage.terminal.search_hint"));
        ...
        this.craftClearButton = this.addRenderableWidget(Button.builder(Component.literal("×"),
                ...
                .tooltip(net.minecraft.client.gui.components.Tooltip.create(
                        Component.translatable("container.immortalstorage.terminal.clear_crafting")))
                .build());
        ...
        this.craftMatchComponentsButton = this.addRenderableWidget(Button.builder(
                ...
                .tooltip(net.minecraft.client.gui.components.Tooltip.create(
                        Component.translatable("container.immortalstorage.terminal.craft_match_components_hint")))
                .build());
        ...
        this.sortDirectionButton = this.addRenderableWidget(Button.builder(directionLabel(), button -> toggleDirection())
                ...
                .tooltip(net.minecraft.client.gui.components.Tooltip.create(
                        Component.translatable("container.immortalstorage.terminal.sort_direction")))
                .build());
        this.fewerRowsButton = this.addRenderableWidget(Button.builder(Component.literal("-"), button -> changeRows(-1))
                .bounds(railX, controlY + TerminalLayout.CONTROL_SIZE * 2,
                        TerminalLayout.CONTROL_SIZE, TerminalLayout.CONTROL_SIZE)
                .tooltip(net.minecraft.client.gui.components.Tooltip.create(
                        Component.translatable("container.immortalstorage.terminal.rows_less")))
                .build());
        this.moreRowsButton = this.addRenderableWidget(Button.builder(Component.literal("+"), button -> changeRows(1))
                .bounds(railX, controlY + TerminalLayout.CONTROL_SIZE * 3,
                        TerminalLayout.CONTROL_SIZE, TerminalLayout.CONTROL_SIZE)
                .tooltip(net.minecraft.client.gui.components.Tooltip.create(
                        Component.translatable("container.immortalstorage.terminal.rows_more")))
                .build());
        updateRowButtons();
        synchronizeViewport();
    }
    ...
    protected final void renderTerminalChrome(GuiGraphicsExtractor graphics, boolean xianqiao) {
        tickScrollAnimation();
    ...
    protected final void renderStorageSlotsClipped(GuiGraphicsExtractor graphics, int storageSlotCount) {
     ...
     */
    protected final void renderStorageAmountOverlays(GuiGraphicsExtractor graphics) {
        ...
        try {
            ...
            for (int row = window.fromInclusive(); row < window.toExclusive(); row++) {
                for (int column = 0; column < TerminalLayout.COLUMNS; column++) {
                    ...
                    graphics.text(this.font, label, -width, -this.font.lineHeight,
                            0xFFFFFFFF, true);
    ...
    protected final boolean renderExactStorageTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (this.searchBox != null && this.searchBox.isFocused() && this.minecraft != null
            ...
            this.searchBox.keyPressed(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, modifiers));
            return true;
        }
    ...
    protected void onViewportChanged(int rows, int baseRow) {
        ...
        ClientPacketDistributor.sendToServer(new ModPayloads.SetTerminalViewport(rows, baseRow));
    }
    ...
    @Override
    protected void renderSlotContents(GuiGraphicsExtractor graphics, ItemStack stack, Slot slot,
    ...
    private void renderSlotHighlightCompat(GuiGraphicsExtractor graphics, Slot slot, int mouseX, int mouseY, float partialTick) {
        int menuIndex = menuIndexOf(slot);
        if (!shouldRenderMenuSlot(menuIndex)) {
            return;
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontal, double vertical) {
        ...
        if (!overTerminal) {
            for (Rect2i extraArea : immortalstorage$getExtraAreas()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        Slot visualSlot = immortalstorage$getSlotAt(mouseX, mouseY);
    ...
    private void sendXianqiaoEntryAction(XianqiaoStorageMenu xianqiao, Slot visualSlot, int button) {
        ...
        ClientPacketDistributor.sendToServer(new ModPayloads.TerminalEntryAction(
    ...
    @Override
    public Slot immortalstorage$getSlotAt(double mouseX, double mouseY) {
    ...
    @Override
    public Rect2i immortalstorage$getSlotBounds(Slot slot) {
        int menuIndex = menuIndexOf(slot);
        int x = this.leftPos + visualSlotX(menuIndex, slot);
        int y = this.topPos + visualSlotY(menuIndex, slot);
        if (isStorageMenuIndex(menuIndex)) {
    ...
    @Override
    public boolean immortalstorage$isCraftingVisible() {
        return this.craftingVisible;
    }
    ...
    private void sendQuery(TerminalQuery query, boolean pushToRecipeViewers) {
        setLocalQuery(query);
        ClientPacketDistributor.sendToServer(new ModPayloads.SetTerminalQuery(
    ...
    private Component sortTooltip() {
        String key = switch (this.sortOrder) {
            case AMOUNT -> "container.immortalstorage.terminal.sort_amount";
            case NAME -> "container.immortalstorage.terminal.sort_name";
            case MOD_ID -> "container.immortalstorage.terminal.sort_mod";
        };
