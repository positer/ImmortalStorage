...
public final class YuanAccount {
    ...
    public void load(CompoundTag tag, YuanProfile activeProfile) {
        if (tag == null) {
            ...
            trueProgress = Math.max(0, tag.getIntOr("trueProgress", 0));
            immortalProgress = Math.max(0, tag.getIntOr("immortalProgress", 0));
    ...
    private static long readLegacyPending(CompoundTag tag, String pendingKey, String oldBalanceKey) {
        if (tag.contains(pendingKey)) {
            return nonNegative(tag.getLongOr(pendingKey, 0L));
        }
        return nonNegative(tag.getLongOr(oldBalanceKey, 0L));
    }
