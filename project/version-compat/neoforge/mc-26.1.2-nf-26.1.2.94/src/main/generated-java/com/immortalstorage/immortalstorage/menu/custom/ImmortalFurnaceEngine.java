...
public final class ImmortalFurnaceEngine {
    ...
    private final Identifier[] activeRecipes = new Identifier[CHANNEL_COUNT];
    @SuppressWarnings("unchecked")
    private final Map<Identifier, Integer>[] recipeUsage = new Map[] {
    ...
    public Identifier activeRecipe(int channel) {
        checkChannel(channel);
        return activeRecipes[channel];
    }

    public void setActiveRecipe(int channel, Identifier value) {
        checkChannel(channel);
        activeRecipes[channel] = value;
    }

    public Map<Identifier, Integer> recipeUsage(int channel) {
        checkChannel(channel);
        return Map.copyOf(recipeUsage[channel]);
    }

    public void setRecipeUsage(int channel, Map<Identifier, Integer> value) {
        checkChannel(channel);
        recipeUsage[channel].clear();
        Map<Identifier, Integer> sanitized = value == null ? Collections.emptyMap() : value;
        sanitized.forEach((id, count) -> {
            if (id != null && count != null && count > 0) recipeUsage[channel].put(id, count);
        });
    }

    public Map<Identifier, Integer> combinedRecipeUsage() {
        Map<Identifier, Integer> combined = new LinkedHashMap<>();
        for (Map<Identifier, Integer> channel : recipeUsage) {
    ...
    public void clearRecipeUsage() {
        for (Map<Identifier, Integer> channel : recipeUsage) channel.clear();
    }
    ...
    public record RecipePlan(Identifier id, ItemStack result, boolean cycleInput) {
        public RecipePlan(Identifier id, ItemStack result) {
