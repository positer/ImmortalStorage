...
public final class WorldShardOutputRouter {
    ...
    public static RouteResult routeCache(List<ItemStack> outputs, @Nullable WorldShardMinerCache cache) {
        ...
        try {
            ...
            ImmortalStorageMod.LOG.error("World shard output cache transaction failed", failure);
            committed = false;
        }
        return committed
    ...
    public static RouteResult routeDirect(
        ...
        ImmortalStoragePlayerData data = endpoint.data();
        ...
        CommitResult commit = data.batchXianqiaoMutations(() -> {
            ...
            try {
                ...
                data.replaceStorage(true, before);
                ImmortalStorageMod.LOG.error("World shard output Xianqiao transaction rolled back", failure);
                return new CommitResult(false, 0L);
            }
        });
    ...
    private static boolean simulatesCompletely(TerminalItemStorage storage, List<BatchEntry> batch) {
        try {
            ...
            ImmortalStorageMod.LOG.error("World shard output Xianqiao preflight failed", failure);
            return false;
        }
    }

    private static boolean fitsYuanCapacity(ImmortalStoragePlayerData data, List<BatchEntry> batch) {
    ...
    private static long remainingYuanCapacity(ImmortalStoragePlayerData data, YuanKind kind) {
