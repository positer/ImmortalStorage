...
/**
 * Advanced Xianqiao Interface screen. Mirrors the plain Xianqiao Interface
 * screen: the nine cache slots, the six-face PULL/PUSH/DISABLED button grid and
 * the left-side active pull/push toggles (only the adjacent-block preview is
 * omitted inside the face buttons). A configuration page tag (⚙) opens the
 * advanced stabilized ruin style settings page (xyz/+xzy range, frequency,
 * preview, enabled, access/split/order), which intentionally omits the six-face
 * grid.
 */
public final class AdvancedXianqiaoInterfaceScreen
        extends com.immortalstorage.immortalstorage.compat.mc2612.CompatAbstractContainerScreen<AdvancedXianqiaoInterfaceMenu>
    ...
    private static final int PANEL_X = SCREEN_WIDTH + 6;
    ...
    public AdvancedXianqiaoInterfaceScreen(
            AdvancedXianqiaoInterfaceMenu menu, Inventory inventory, Component title) {
    ...
    protected void init() {
        ...
        for (int index = 0; index < SIDE_ORDER.size(); index++) {
            ...
            FacePreviewButton button = new FacePreviewButton(
                    this.leftPos + SIDE_GRID_X + (index % 3) * SIDE_COLUMN_STRIDE,
                    this.topPos + SIDE_GRID_Y + (index / 3) * SIDE_ROW_STRIDE,
                    SIDE_BUTTON_WIDTH, Component.literal(shortSide(side)),
                    () -> ItemStack.EMPTY, () -> interfaceModeColor(side),
        ...
        addRenderableWidget(Button.builder(Component.literal("⚙"), clicked -> {
                    settings = !settings;
                    refreshWidgets();
                }).bounds(leftPos + 4, topPos - 20, 20, 20)
                .tooltip(Tooltip.create(Component.translatable(
                        "container.immortalstorage.advanced_xianqiao_interface.config")))
                .build());
        if (settings) addSettingsWidgets();
        ...
        for (int index = 0; index < SIDE_ORDER.size(); index++) {
            Direction side = SIDE_ORDER.get(index);
            Button maskButton = new FacePreviewButton(
                    dialogX + 42 + (index % 3) * 32,
                    dialogY + 62 + (index / 3) * 20, MASK_BUTTON_SIZE,
                    Component.literal(shortSide(side)), () -> ItemStack.EMPTY,
    ...
    private void addSettingsWidgets() {
        ...
        for (int index = 0; index < 6; index++) {
            ...
            addActionButton(leftPos + PANEL_X, y, 16, 16, "-", index * 2);
            addActionButton(leftPos + PANEL_X + 72, y, 16, 16, "+", index * 2 + 1);
            int valueIndex = index;
            EditBox box = new EditBox(font, leftPos + PANEL_X + 19, y, 50, 16, Component.empty());
            ...
            box.setResponder(text -> {
                ...
                try {
                    ClientPacketDistributor.sendToServer(new ModPayloads.SetStabilizedRuinValue(
                            menu.containerId, valueIndex, Integer.parseInt(text)));
        ...
        previewButton = addActionButton(leftPos + PANEL_X, topPos + 130, 88, 18,
                Component.translatable("container.immortalstorage.ruin.preview").getString(), 12);
        enabledButton = addActionButton(leftPos + PANEL_X, topPos + 150, 88, 18,
                Component.translatable("container.immortalstorage.ruin.enabled").getString(), 13);
        addActionButton(leftPos + PANEL_X, topPos + 170, 20, 18, "-", 14);
        addActionButton(leftPos + PANEL_X + 68, topPos + 170, 20, 18, "+", 15);
        EditBox frequency = new EditBox(font, leftPos + PANEL_X + 23, topPos + 170, 42, 18, Component.empty());
        ...
        frequency.setResponder(text -> {
            ...
            try {
                ClientPacketDistributor.sendToServer(new ModPayloads.SetStabilizedRuinValue(
                        menu.containerId, 6, Integer.parseInt(text)));
            } catch (NumberFormatException ignored) {
            }
        });
        valueBoxes.add(frequency);
        addRenderableWidget(frequency);
        accessButton = addActionButton(leftPos + PANEL_X, topPos + 196, 88, 18,
                Component.translatable(menu.value(9) == 0
                        ? "container.immortalstorage.advanced_ruin.access_skip"
                        : "container.immortalstorage.advanced_ruin.access_force").getString(), 16);
        splitButton = addActionButton(leftPos + PANEL_X, topPos + 216, 88, 18,
                Component.translatable(menu.value(10) == 0
                        ? "container.immortalstorage.advanced_ruin.split_item"
                        : "container.immortalstorage.advanced_ruin.split_group").getString(), 17);
        orderButton = addActionButton(leftPos + PANEL_X, topPos + 236, 88, 18,
                Component.translatable(menu.value(11) == 0
                        ? "container.immortalstorage.advanced_ruin.order_far"
                        : "container.immortalstorage.advanced_ruin.order_near").getString(), 18);
    }
    ...
    protected void renderBg(GuiGraphicsExtractor graphics, float partialTick, int mouseX, int mouseY) {
        ...
        for (int column = 0; column < 9; column++) {
            ...
            VanillaGuiPainter.slot(graphics, slotX, y + AdvancedXianqiaoInterfaceMenu.CONFIG_Y, true);
            graphics.fill(slotX, y + AdvancedXianqiaoInterfaceMenu.CONFIG_Y,
                    slotX + 16, y + AdvancedXianqiaoInterfaceMenu.CONFIG_Y + 16, GHOST_TINT);
            VanillaGuiPainter.slot(graphics, slotX, y + AdvancedXianqiaoInterfaceMenu.BUFFER_Y, true);
        }
        int selectedX = x + 8 + selectedTarget * 18;
        graphics.outline(selectedX - 1, y + AdvancedXianqiaoInterfaceMenu.CONFIG_Y - 1,
                18, 18, 0xFF63B7BE);
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                VanillaGuiPainter.slot(graphics, x + 8 + column * 18,
                        y + AdvancedXianqiaoInterfaceMenu.PLAYER_INVENTORY_Y + row * 18, true);
            }
        }
        for (int column = 0; column < 9; column++) {
            VanillaGuiPainter.slot(graphics, x + 8 + column * 18,
                    y + AdvancedXianqiaoInterfaceMenu.HOTBAR_Y, true);
        }
        if (settings) renderSettingsPanel(graphics);
    }

    private void renderSettingsPanel(GuiGraphicsExtractor graphics) {
        String[] labels = {"x", "y", "z", "+x", "+y", "+z"};
        for (int i = 0; i < labels.length; i++) {
            graphics.text(font, labels[i], leftPos + PANEL_X + 92, topPos + 22 + i * 18, 0xFFFFFF, true);
        }
        graphics.text(font, Component.translatable("container.immortalstorage.ruin.tick").getString(),
                leftPos + PANEL_X + 92, topPos + 175, 0xFFFFFF, true);
    }
    ...
    public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (amountDialogOpen) {
            ...
            if (menu.getCarried().isEmpty() && externalHover.isPresent()) {
                ...
                com.immortalstorage.immortalstorage.compat.mc2612.CompatGui.renderTooltip(graphics, this.font, List.of(hover.stack().getHoverName(),
    ...
    private Optional<ExternalResourceHover> externalResourceHover() {
        ...
        if (slotIndex < 0 || slotIndex >= AdvancedXianqiaoInterfaceMenu.PLAYER_START) {
            return Optional.empty();
        }
        int resourceSlot = slotIndex % AdvancedXianqiaoInterfaceMenu.CONFIG_SLOT_COUNT;
        ...
        long amount = slotIndex < AdvancedXianqiaoInterfaceMenu.CONFIG_SLOT_COUNT
    ...
    protected void renderSlot(GuiGraphicsExtractor graphics, Slot slot) {
        ...
        int slotIndex = this.menu.slots.indexOf(slot);
        if (slotIndex < AdvancedXianqiaoInterfaceMenu.BUFFER_START
                || slotIndex >= AdvancedXianqiaoInterfaceMenu.PLAYER_START
                || !slot.getItem().isEmpty()) return;
        int resourceSlot = slotIndex - AdvancedXianqiaoInterfaceMenu.BUFFER_START;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int relativeY = (int) mouseY - this.topPos - AdvancedXianqiaoInterfaceMenu.CONFIG_Y;
    ...
    private void updateControls() {
        for (Map.Entry<Direction, FacePreviewButton> entry : sideButtons.entrySet()) {
            ...
            sideButton.setMessage(Component.literal(shortSide(entry.getKey())));
    ...
    private Component sideTooltip(Direction side) {
        return Component.translatable("container.immortalstorage.xianqiao_interface.side_tooltip",
                Component.translatable(
                        "container.immortalstorage.xianqiao_interface.side_full." + side.getName()),
    ...
    private void openAmountDialog(int slot) {
        ...
        amountInput.setFilter(fluid
                ? AdvancedXianqiaoInterfaceScreen::validBucketInputShape
    ...
    private void configureExternalResource(ResourceChannelKey key) {
        ...
        ClientPacketDistributor.sendToServer(new ModPayloads.SetXianqiaoInterfaceExternalTarget(
                ...
                AdvancedXianqiaoInterfaceMenu.DEFAULT_EXTERNAL_CACHE_AMOUNT));
        closeExternalResourceDialog();
    }
    ...
    public List<TerminalFluidScreenAccess.FluidHover> immortalstorage$getVisibleFluids() {
        ...
        for (int slot = 0; slot < AdvancedXianqiaoInterfaceMenu.CONFIG_SLOT_COUNT; slot++) {
            ...
            if (desired > 0L) {
                result.add(new TerminalFluidScreenAccess.FluidHover(
                        ...
                        this.topPos + AdvancedXianqiaoInterfaceMenu.CONFIG_Y, 16, 16)));
            }
            long cached = menu.getCachedAmount(slot);
            if (cached > 0L) {
                result.add(new TerminalFluidScreenAccess.FluidHover(
                        fluid, cached, new Rect2i(x,
                        this.topPos + AdvancedXianqiaoInterfaceMenu.BUFFER_Y, 16, 16)));
            }
        }
    ...
    private FluidStack configuredFluidIdentity(int slot) {
        ...
        if (!marker.contains(AdvancedXianqiaoInterfaceMenu.FLUID_DISPLAY_TAG)) return FluidStack.EMPTY;
        return com.immortalstorage.immortalstorage.compat.mc2612.CompatCodec.parseFluidStack(this.minecraft.level.registryAccess(),
                marker.getCompoundOrEmpty(AdvancedXianqiaoInterfaceMenu.FLUID_DISPLAY_TAG));
    }
