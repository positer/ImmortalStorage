...
public class KongqiaoMenu extends AbstractContainerMenu implements StorageTerminalView, CraftingTransferTarget, SmithingTransferTarget {
    ...
    public void clicked(int slotId, int button, ContainerInput clickType, Player actor) {
        ...
        if (slotId >= 0 && slotId < VISIBLE_STORAGE_SLOTS) {
            ...
            if (clickType == ContainerInput.PICKUP && this.getCarried().isEmpty() && !stored.isEmpty()) {
            ...
            if (clickType == ContainerInput.PICKUP && !stored.isEmpty()
            ...
            if (clickType == ContainerInput.THROW && this.getCarried().isEmpty() && !stored.isEmpty()) {
            ...
            if (clickType == ContainerInput.SWAP && !stored.isEmpty()
                    && stored.getCount() > stored.getMaxStackSize()) {
    ...
    private void refreshCraftingResult(RecipeHolder<CraftingRecipe> lastRecipe) {
        ...
        if (recipe.isPresent() && this.resultSlots.setRecipeUsed(serverPlayer, recipe.get())) {
