...
public class XianqiaoInterfaceMenu extends AbstractContainerMenu {
    ...
    public void clicked(int slotId, int button, ContainerInput clickType, Player player) {
        if (slotId >= 0 && slotId < CONFIG_SLOT_COUNT) {
            if (clickType != ContainerInput.PICKUP || !hasLiveAccess(player)) return;
        ...
        if (slotId >= BUFFER_START && slotId < PLAYER_START) {
            ...
            if (clickType == ContainerInput.PICKUP && !getCarried().isEmpty()) {
    ...
    public ResourceChannelKey getExternalTarget(int slot) {
        ...
        if (!marker.contains(EXTERNAL_DISPLAY_TAG)) return null;
        CompoundTag external = marker.getCompoundOrEmpty(EXTERNAL_DISPLAY_TAG);
        try {
            return new ResourceChannelKey(external.getStringOr("Channel", ""), external.getStringOr("Resource", ""));
    ...
    private ItemStack displayTarget(XianqiaoInterfaceInventory backend, int slot) {
        ...
        if (blockEntity != null && blockEntity.getLevel() != null) {
            Tag encoded = com.immortalstorage.immortalstorage.compat.mc2612.CompatCodec.saveFluidStack(blockEntity.getLevel().registryAccess(), fluid.copyWithAmount(1));
