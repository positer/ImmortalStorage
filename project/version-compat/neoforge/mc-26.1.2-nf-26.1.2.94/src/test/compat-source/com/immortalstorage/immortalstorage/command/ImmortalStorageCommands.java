...
public final class ImmortalStorageCommands {
    ...
    public static void register(RegisterCommandsEvent event) {
        var root = Commands.literal("immortalstorage").requires(source -> com.immortalstorage.immortalstorage.compat.mc2612.CompatCommands.hasPermission(source, 2));
    ...
    private static int unloadRealm(ServerPlayer player,
        ...
        com.immortalstorage.immortalstorage.dimension.RealmHelper.suspendRealmLoading(
                com.immortalstorage.immortalstorage.compat.mc2612.CompatLevel.server(player.level()), com.immortalstorage.immortalstorage.dimension.RealmHelper.realmId(player));
        context.getSource().sendSuccess(() -> Component.literal(
                "Suspended Xianqiao realm loading for " + player.getGameProfile().name()), true);
        return 1;
    }
    ...
    private static int reloadRealm(ServerPlayer player,
        ...
        context.getSource().sendSuccess(() -> Component.literal(
                "Reloaded Xianqiao realm loading for " + player.getGameProfile().name()), true);
        return 1;
    }
    ...
    private static int setSpeed(ServerPlayer player, String rawSpeed,
        ...
        context.getSource().sendSuccess(() -> Component.literal(
                "Set " + player.getGameProfile().name() + " Xianqiao speed to "
