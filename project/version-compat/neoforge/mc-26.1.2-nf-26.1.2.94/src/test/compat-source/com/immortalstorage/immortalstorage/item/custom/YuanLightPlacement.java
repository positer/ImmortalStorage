...
final class YuanLightPlacement {
    static InteractionResult use(Level level, Player player, InteractionHand hand, boolean immortal) {
        ...
        if (!level.getBlockState(pos).canBeReplaced() || !level.getWorldBorder().isWithinBounds(pos)
                || !level.mayInteract(player, pos)) return InteractionResult.FAIL;
