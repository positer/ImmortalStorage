...
final class AdvancedXianqiaoInterfaceScheduler {
    ...
    private static boolean pullItems(AdvancedXianqiaoInterfaceBlockEntity be, ServerLevel level,
        ...
        IItemHandler handler = com.immortalstorage.immortalstorage.compat.mc2612.CompatTransfer.itemHandler(level.getCapability(Capabilities.Item.BLOCK, pos, face));
    ...
    private static void pullFluids(AdvancedXianqiaoInterfaceBlockEntity be, ServerLevel level,
        ...
        IFluidHandler source = com.immortalstorage.immortalstorage.compat.mc2612.CompatTransfer.fluidHandler(level.getCapability(
                Capabilities.Fluid.BLOCK, pos, face));
    ...
    private static void pullEnergy(AdvancedXianqiaoInterfaceBlockEntity be, ServerLevel level,
                                   BlockPos pos, Direction face) {
        IEnergyStorage source = com.immortalstorage.immortalstorage.compat.mc2612.CompatTransfer.energyHandler(level.getCapability(
                Capabilities.Energy.BLOCK, pos, face));
        if (source == null) return;
    ...
    private static ItemStack distributeItem(ItemStack stack, ServerLevel level,
        ...
        if (!itemByItem) {
            for (int i = 0; i < count; i++) {
                ...
                IItemHandler handler = com.immortalstorage.immortalstorage.compat.mc2612.CompatTransfer.itemHandler(level.getCapability(
                        Capabilities.Item.BLOCK, pos, face));
                if (handler == null) {
    ...
    private static FluidStack distributeFluid(FluidStack stack, ServerLevel level,
        ...
        for (BlockPos pos : targets) {
            ...
            IFluidHandler target = com.immortalstorage.immortalstorage.compat.mc2612.CompatTransfer.fluidHandler(level.getCapability(
                    Capabilities.Fluid.BLOCK, pos, face));
    ...
    private static void pushEnergy(AdvancedXianqiaoInterfaceBlockEntity be, ServerLevel level,
        ...
        for (BlockPos pos : targets) {
            ...
            IEnergyStorage target = com.immortalstorage.immortalstorage.compat.mc2612.CompatTransfer.energyHandler(level.getCapability(
                    Capabilities.Energy.BLOCK, pos, face));
            if (target == null) continue;
