...
public final class WorldShardMinerModeParser {
    ...
    public static WorldShardMinerMode parse(Identifier source, JsonObject json) {
        requireKnownFields(source, json, FIELDS, "mode");
        Identifier id = parseId(source, GsonHelper.getAsString(json, "id"), "id");
        ...
        if (hasBlock) {
            Identifier blockId = parseId(source,
        ...
        Optional<Identifier> targetDimension = json.has("target_dimension")
        ...
        Map<Identifier, Long> weights = parseWeights(source, json);
    ...
    private static Map<Identifier, Long> parseWeights(Identifier source, JsonObject json) {
        if (!json.has("ore_weights")) return Map.of();
        JsonObject weightsJson = GsonHelper.getAsJsonObject(json, "ore_weights");
        Map<Identifier, Long> weights = new LinkedHashMap<>();
        for (Map.Entry<String, JsonElement> entry : weightsJson.entrySet()) {
            Identifier itemId = parseId(source, entry.getKey(), "ore_weights key");
    ...
    private static Identifier parseId(Identifier source, String raw, String field) {
        Identifier id = Identifier.tryParse(raw);
        if (id == null) throw failure(source, field + " is not a valid resource location: " + raw);
        return id;
    }

    private static void requireKnownFields(Identifier source, JsonObject json,
    ...
    private static IllegalArgumentException failure(Identifier source, String message) {
