...
public class XianqiaoStorageScreen extends AbstractTerminalScreen<XianqiaoStorageMenu>
    ...
    protected void init() {
        ...
        this.slowerTimeButton = this.addRenderableWidget(Button.builder(Component.literal("-"),
                        button -> ClientPacketDistributor.sendToServer(new ModPayloads.TimeFlow(this.menu.containerId, -1)))
    ...
    protected void renderBg(GuiGraphicsExtractor graphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (this.realmVisible) {
            ...
            graphics.text(this.font, timeScaleTitle,
            ...
            graphics.text(this.font, timeScaleValue,
    ...
    public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (fluidHover.isPresent()) {
            ...
            com.immortalstorage.immortalstorage.compat.mc2612.CompatGui.renderTooltip(graphics, this.font, List.of(
                            ...
                            Component.literal(TerminalFluidAmountFormatter.exactMillibuckets(hover.amountMb()))), mouseX, mouseY);
        } else {
            ...
            if (external != null) {
                ...
                com.immortalstorage.immortalstorage.compat.mc2612.CompatGui.renderTooltip(graphics, this.font, List.of(
                        ...
                        Component.literal(Long.toString(external.amount())
                                + (definition.unit().isEmpty() ? "" : " " + definition.unit()))), mouseX, mouseY);
                return;
            }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0 || button == 1) {
            ...
            if (cell != null) {
                if (cell.entry() != null) {
                    ClientPacketDistributor.sendToServer(new ModPayloads.TerminalFluidEntryAction(
                ...
                if (external != null) {
                    ClientPacketDistributor.sendToServer(new ModPayloads.TerminalExternalResourceEntryAction(
    ...
    private void renderFluidStorage(GuiGraphicsExtractor graphics) {
    ...
    private void renderExternalResourceStorage(GuiGraphicsExtractor graphics) {
        Rect2i clip = storageBounds();
        graphics.enableScissor(clip.getX(), clip.getY(),
                clip.getX() + clip.getWidth(), clip.getY() + clip.getHeight());
        try {
            ...
            for (int row = window.fromInclusive(); row < window.toExclusive(); row++) {
                for (int column = 0; column < TerminalLayout.COLUMNS; column++) {
                    ...
                    if (definition.solidColor()) {
                        ...
                        com.immortalstorage.immortalstorage.compat.mc2612.CompatGui.blitTexture(graphics, definition.icon(), fluidVisualX(index), fluidVisualY(index),
                                0.0F, 0.0F, 16, 16, 16,
    ...
    private void renderExternalResourceAmountOverlays(GuiGraphicsExtractor graphics) {
        Rect2i clip = storageBounds();
        boolean needsScissor = fractionalScrollOffset() != 0;
    ...
    private void renderFluidAmountOverlays(GuiGraphicsExtractor graphics) {
        Rect2i clip = storageBounds();
        boolean needsScissor = fractionalScrollOffset() != 0;
        if (needsScissor) {
    ...
    private void renderFluidAmount(GuiGraphicsExtractor graphics, int viewIndex, long amountMb, int x, int y) {
