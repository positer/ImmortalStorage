...
public final class XianqiaoRedstoneInterfaceMenu extends AbstractContainerMenu {
    ...
    public void clicked(int slotId, int button, ContainerInput clickType, Player player) {
        if (slotId == 0) {
            if (clickType != ContainerInput.PICKUP || blockEntity == null || !stillValid(player)) return;
    ...
    public ResourceChannelKey getExternalTarget() {
        ...
        CompoundTag encoded = marker.getCompoundOrEmpty(EXTERNAL_DISPLAY_TAG);
        try {
            return new ResourceChannelKey(
                    encoded.getStringOr("Channel", ""), encoded.getStringOr("Resource", ""));
