...
final class XianqiaoRsStorage implements SerializableStorage, CompositeAwareChild {
    ...
    @Override
    public com.refinedmods.refinedstorage.common.api.storage.StorageContents toContents() {
        List<com.refinedmods.refinedstorage.common.api.storage.StorageContents.Stored> stored = getAll().stream()
                .map(entry -> new com.refinedmods.refinedstorage.common.api.storage.StorageContents.Stored(entry.resource(), entry.amount(), java.util.Optional.empty()))
                .toList();
        return new com.refinedmods.refinedstorage.common.api.storage.StorageContents(getType(), java.util.Optional.empty(), stored);
    }
